...
let rec dyn_of_gen f =
  ...
  | Lookbehind (pn, t) ->
    let pn =
      ...
      | `Neg -> variant "Neg" []
    in
    variant "Lookbehind" [ pn; dyn_of_gen f t ]
...
let rec to_api cset_to_api t =
  ...
  | Lookahead (pn, t1) | Lookbehind (pn, t1) ->
    call'
      (match t with
       | Lookahead _ -> "lookahead"
       | Lookbehind _ -> "lookbehind"
       | _ -> assert false)
      [ ident
          (match pn with
           | `Pos -> "`Pos"
...
let rec handle_case ign_case : t -> (Cset.t, [ `Uncased ]) gen = function
  ...
  | Lookbehind (pn, r) -> Lookbehind (pn, handle_case ign_case r)
  | ( Beg_of_line
    | End_of_line
    | Beg_of_word _
    | End_of_word _
...
module Export = struct
  ...
  let lookbehind pn r = Lookbehind (pn, r)
...
let colorize color_map (regexp : no_case) =
  ...
  let rec colorize regexp =
    ...
    | Repeat (r, _, _) | Lookahead (_, r) | Lookbehind (_, r) -> colorize r
